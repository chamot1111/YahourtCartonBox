...
*/
    ...
    function s(d, f) {
        ...
        if ("UTF8" == f)
            for (g = 0; g < d.length; g += 1) {
                ...
                for (b = 0; b < c.length; b += 1) a[e >>> 2] |= c[b] << 24 - 8 * (e % 4), e += 1
            } else if ("UTF16" == f)
                for (g = 0; g < d.length; g += 1) a[e >>> 2] |= d.charCodeAt(g) << 16 - 8 * (e % 4), e += 2;
        return {
            value: a,
            binLen: 8 * e
        }
    ...
    function u(d) {
        ...
        for (b = 0; b < a; b += 2) c = parseInt(d.substr(b, 2), 16), isNaN(c) && k("String of HEX type contains invalid characters"), f[b >>> 3] |= c << 24 - 4 * (b % 8);
        return {
            value: f,
            binLen: 4 * a
        }
    ...
    function v(d) {
        var f = [],
            a = 0,
            b, c, e, g, h; - 1 === d.search(/^[a-zA-Z0-9=+\/]+$/) && k("Invalid character in base-64 string");
        b = d.indexOf("=");
        d = d.replace(/\=/g, ""); - 1 !== b && b < d.length && k("Invalid '=' found in base-64 string");
        for (c = 0; c < d.length; c += 4) {
            h = d.substr(c, 4);
            for (e = g = 0; e < h.length; e += 1) b = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".indexOf(h[e]), g |= b << 18 - 6 * e;
            for (e = 0; e < h.length - 1; e += 1) f[a >> 2] |= (g >>> 16 - 8 * e & 255) << 24 - 8 * (a % 4), a += 1
        }
        return {
            value: f,
            binLen: 8 * a
    ...
    function w(d, f) {
        var a = "",
            b = 4 * d.length,
            c, e;
        for (c = 0; c < b; c += 1) e = d[c >>> 2] >>> 8 * (3 - c % 4), a += "0123456789abcdef".charAt(e >>> 4 & 15) + "0123456789abcdef".charAt(e & 15);
    ...
    function x(d, f) {
        ...
        for (c = 0; c < b; c += 3) {
            ...
            for (e = 0; 4 > e; e += 1) a = 8 * c + 6 * e <= 32 * d.length ? a + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(g >>> 6 * (3 - e) & 63) : a + f.b64Pad
        }
        return a
    }
    ...
    function B(d, f) {
        var a = [],
            b, c, e, g, h, C, t, j, D, l = [1732584193, 4023233417, 2562383102, 271733878, 3285377520],
            n = [1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1518500249, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393, 1859775393,
        ...
        for (t = 0; t < D; t += 16) {
            ...
            g = l[3];
            h = l[4];
            for (j = 0; 80 > j; j += 1) a[j] = 16 > j ? d[j + t] : (a[j - 3] ^ a[j - 8] ^ a[j - 14] ^ a[j - 16]) << 1 | (a[j - 3] ^ a[j - 8] ^ a[j - 14] ^ a[j - 16]) >>> 31, C = 20 > j ? A(b << 5 | b >>> 27, c & e ^ ~c & g, h, n[j], a[j]) : 40 > j ? A(b << 5 | b >>> 27, c ^ e ^ g, h, n[j], a[j]) : 60 > j ? A(b << 5 | b >>> 27, c & e ^ c & g ^ e & g, h, n[j], a[j]) : A(b << 5 | b >>> 27, c ^ e ^ g, h, n[j], a[j]), h = g, g = e, e = c << 30 | c >>> 2, c = b, b = C;
            l[0] = z(b, l[0]);
            l[1] = z(c, l[1]);
            l[2] = z(e, l[2]);
            l[3] = z(g, l[3]);
    ...
    module.exports = function (d, f, a) {
        var b = null,
            c = 0,
            e = [0],
            g = "",
            h = null,
            g = "undefined" !== typeof a ? a : "UTF8";
        ...
        this.getHMAC = function (b, a, d, f, h) {
            ...
            for (m = 0; m <= a; m += 1) G[m] = p[m] ^ 909522486, H[m] = p[m] ^ 1549556828;
